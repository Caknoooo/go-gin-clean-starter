...
init-uuid:
	docker exec -it ${POSTGRES_CONTAINER_NAME} psql -U ${DB_USER} -d ${DB_NAME} -c 'CREATE EXTENSION IF NOT EXISTS "uuid-ossp";'
...
migrate:
	docker exec -it ${CONTAINER_NAME} go run main.go --migrate

seed:
	docker exec -it ${CONTAINER_NAME} go run main.go --seed

migrate-seed:
	docker exec -it ${CONTAINER_NAME} go run main.go --migrate --seed
